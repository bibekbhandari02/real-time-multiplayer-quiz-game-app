        ...
        if (playersJoined >= players.length) {
          ...
          setTimeout(async () => {
            ...
            if (updatedRoom && updatedRoom.status === 'waiting') {
              ...
              console.log(`🎮 Auto-starting ranked match: ${roomCode} (${category}, ${difficultyMode})`);
              
              // Notify players that questions are being generated
              io.to(roomCode).emit('generating_questions', { 
                message: `🤖 AI is generating questions for ${category}...` 
              });
              
              // Generate questions with enhanced AI
              let aiQuestions;
              try {
                console.log(`📝 Generating ${count} questions for ${category} (${difficultyMode} mode)...`);
                aiQuestions = await generateQuestionsWithFallback(category, difficultyMode, count);
                console.log(`✅ AI returned ${aiQuestions?.length || 0} questions`);
              } catch (error) {
                console.error('❌ Error generating questions for ranked match:', error);
              ...
              console.log(`📋 Validating ${aiQuestions.length} questions...`);
              ...
              const validatedQuestions = aiQuestions
                .map(q => {
                  ...
                  if (!validation.valid) {
                    console.warn(`⚠️ Invalid ranked question filtered:`, validation.issues);
                    return null;
                  }
              ...
              console.log(`✅ ${validatedQuestions.length} valid questions ready for ranked match`);
                ...
                console.log(`🚀 Starting ranked match ${roomCode} with ${populatedRoom.questions.length} questions`);
              ...
              console.log(`⚠️ Room ${roomCode} not found or already started`);
            }
          }, 3000);
        }
